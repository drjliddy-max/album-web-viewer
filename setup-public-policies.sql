...
-- This adds public read-only access for shared albums
-- Run this in Supabase SQL Editor

-- Drop existing public policies if they exist
DROP POLICY IF EXISTS "Public can view active shared albums" ON public.shared_albums;
DROP POLICY IF EXISTS "Public can view babies for shared albums" ON public.babies;
DROP POLICY IF EXISTS "Public can view photos in shared albums" ON public.photos;
DROP POLICY IF EXISTS "Public can view pdfs in shared albums" ON public.pdfs;

-- 1. Add public read policy for active shared albums
CREATE POLICY "Public can view active shared albums"
ON public.shared_albums FOR SELECT
TO anon
USING (is_active = true);

-- 2. Add public read policy for babies
CREATE POLICY "Public can view babies for shared albums"
...
CREATE POLICY "Public can view pdfs in shared albums"
...
-- 5. Create the view count increment function
...
-- Success message
SELECT '✅ Public viewing policies created successfully!' as status;
